...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def set_ui(self):
        ...
        # 窗口图标
        self.icon = QIcon()
        self.icon.addPixmap(QPixmap("D:\\project\\Smart_container\\client\\image\\1.png"), QIcon.Normal, QIcon.On)
        ...
        self.title_label.setGeometry(QRect(240*self.rate, 25*self.rate, 500*self.rate, 60*self.rate))
        self.title_label.setStyleSheet("color: #5453FF;"
                                       "background: transparent;"
                                       "font: 20pt \"华光粗圆_CNKI\";"
                                       "font-weight:bold;")
        self.title_label.setText("袋鼯麻麻——智能零售结算平台")

        self.setWindowTitle('袋鼯麻麻——智能零售结算平台')
...
# D:\\project\\Smart_container\\1.png
        # 袋鼯 Logo
        self.daiwu_logo=QLabel("图片",self)
        self.daiwu_pic=QPixmap("D:\\project\\Smart_container\\client\\image\\1.png")
        self.daiwu_logo.setPixmap(self.daiwu_pic)
        self.daiwu_logo.setGeometry(QRect(921*self.rate, 10*self.rate, 190*self.rate, 150*self.rate))
        ...
        self.text.setGeometry(QRect(320*self.rate, 350*self.rate, 420*self.rate, 50*self.rate))
        ...
        self.text.setText("袋鼯麻麻——智能零售结算平台使用指南")
        ...
        self.version_label.setGeometry(QRect(920*self.rate, 580*self.rate, 200*self.rate, 20*self.rate))
        ...
        self.author_label.setGeometry(QRect(920*self.rate, 600*self.rate, 200*self.rate, 80*self.rate))
    ...
    def container_recognition(self):
        ...
        self.picture_file = 'D:\\project\\Smart_container\\client\\test_client_pic.jpg'

        cv2.imwrite(self.picture_file, self.image)

        img_str = self.getByte('D:\\project\\Smart_container\\client\\test_client_pic.jpg')

        requestsss={'name':'测试图片', 'image':img_str}
        req = json.dumps(requestsss) #字典数据结构变json(所有程序语言都认识的字符串)

        res=requests.post('http://47.100.88.229:8001/reference_client/', data=req)
        print(type(res.text))
